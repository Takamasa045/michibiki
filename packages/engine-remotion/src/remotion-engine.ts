...
import {
  ...
  buildStandaloneRoot,
  buildStandaloneTsConfig
} from "./standalone-templates.js";
...
async function renderRemotionProject(
  ...
  const loggedResult = { ...result, stdout: `${result.stdout}${copyMessage}` };
  await writeLog(context.logDir, "render.log", loggedResult);
