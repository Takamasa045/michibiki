...
import {
  getCapabilitiesForEngine,
  getRouterSignals,
  type RouterSignals
} from "./signals.js";

// Max fit-percent gap between the top and runner-up engine that still counts
// as a "close call": at or below this, the router emits a clarifying question
// and flags the selection guide. Single source of truth for the CLI help text
// and the router's own thresholds.
...
function buildClarifyingQuestions(
  ...
  if (margin > AMBIGUITY_MARGIN) return [];
...
function buildSelectionGuide(
  ...
  const closeCallNotice =
    runnerUp && selectedFit.fitPercent - runnerUp.fitPercent <= AMBIGUITY_MARGIN
